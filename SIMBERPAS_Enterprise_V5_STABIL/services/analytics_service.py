...
def dashboard_metrics(df: pd.DataFrame) -> DashboardMetrics:
    ...
    return DashboardMetrics(
        ...
        high=int((out["urgensi"] == "Tinggi").sum()),
...
def deterministic_summary(df: pd.DataFrame) -> tuple[str, str, str]:
    ...
    if m.high > 0 or m.negative >= 5:
        return summary, "TINGGI", "Lakukan telaah segera dan koordinasikan verifikasi dengan UPT terkait."
    if m.negative > 0:
        return summary, "SEDANG", "Pantau perkembangan berita negatif dan pastikan data telah diverifikasi."
