...
def get_telegram_config() -> tuple[str | None, str | None]:
    """
    Mengambil Telegram Bot Token dan Chat ID dari st.secrets atau session.
    Pastikan Anda sudah menambahkan konfigurasi di .streamlit/secrets.toml:
    [telegram]
...
def send_telegram_message(text: str, parse_mode: str = "HTML") -> bool:
    ...
    if not bot_token or not chat_id:
        logger.warning("Telegram config (bot_token / chat_id) belum diatur di secrets.toml.")
        return False
...
def send_telegram_document(
    ...
    """Mengirim berkas dokumen (PDF/Excel) ke grup Telegram monitoring."""
    bot_token, chat_id = get_telegram_config()
    if not bot_token or not chat_id:
        logger.warning("Telegram config (bot_token / chat_id) belum diatur di secrets.toml.")
        return False
