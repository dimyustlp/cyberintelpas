...
def attach_v6_pages(pages: dict[str, list[st.Page]], user: Any) -> dict[str, list[st.Page]]:
    """Menambahkan modul V6 tanpa mengganti halaman lama yang sudah stabil."""
    pages.setdefault("Briefing & Intelijen", [])
    _append_once(
        pages["Briefing & Intelijen"],
    ...
    if has_permission(user, "view_system_health") or has_permission(user, "manage_users"):
        admin = pages.setdefault("Administrasi", [])
